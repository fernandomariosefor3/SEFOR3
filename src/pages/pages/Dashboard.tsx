      ...
      <div className="bg-white rounded-2xl p-6 shadow border-b-4 border-[#ca8a04]">
        ...
        <ResponsiveContainer width="100%" height={300}>
          <BarChart data={escolas} margin={{ top: 5, right: 20, left: 0, bottom: 60 }}>
            ...
            <Tooltip
              contentStyle={{ borderRadius: '12px', border: '2px solid #ca8a04' }}
            />
            ...
            <tbody>
              {ranking.map((escola, i) => (
                <tr key={escola.nome} className="border-b border-gray-100 hover:bg-green-50 transition-colors">
                  ...
                  <td className="p-3 font-bold">{escola.nome}</td>
                  <td className="p-3 text-center">
                    <span className={`px-3 py-1 rounded-full text-white text-xs font-black
                      ${escola.frequencia >= 90 ? 'bg-green-500' : escola.frequencia >= 80 ? 'bg-yellow-500' : 'bg-red-500'}`}>
                      {escola.frequencia}%
                    </span>
                  </td>
                  <td className="p-3 text-center font-bold text-gray-700">{escola.notas}%</td>
                  <td className="p-3 text-center font-bold text-purple-600">{escola.projetos}</td>
                </tr>
              ))}
            </tbody>
          </table>
